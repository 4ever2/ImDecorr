...
public class utils {
	...
	// This class contains generic utility Image Processing functions required for
	// the analysis
	...
	static public FloatProcessor getPreprocessedImage(FloatProcessor im) {
		...
		// Since the image is already apodized, the padding value is given by any edge
		// pixel
		float meanIm = pixelsIn[0];

		float[] pixelsOut = new float[newSize * newSize];
		for (int k = 0; k < newSize * newSize; k++) {
			pixelsOut[k] = meanIm;
		}
	...
	static public FloatProcessor apodizeEdges(FloatProcessor im) {
		...
		for (int x = 0; x < im.getWidth(); x++)
			for (int y = 0; y < im.getHeight(); y++) {
				dist = (x - im.getWidth() / 2) * (x - im.getWidth() / 2)
						+ (y - im.getHeight() / 2) * (y - im.getHeight() / 2);
				if (dist > r * r) {
		...
		for (int x = 0; x < im.getWidth(); x++)
			for (int y = 0; y < im.getHeight(); y++) {
				...
				} else if (Math.abs(x - im.getWidth() / 2) > im.getWidth() / 2
						& Math.abs(y - im.getHeight() / 2) > im.getHeight() / 2) {
	...
	// map the input value val, which belongs to [valmin,valmax] to the range
	// [mapmin,mapmax]
