...
public class output {
    ...
    static String getID(int n) {
        ...
        String m = String.format("%02d", Integer.parseInt(Integer.toString(cal.get(Calendar.MONTH) + 1))); // Month
                                                                                                           // starts
                                                                                                           // from 0 to
                                                                                                           // 11
    ...
    static void plotResults(ImageDecorrelationAnalysis ida) {
        ...
        if (ida.kcMax < 0.7) {
            p.addLabel(ida.kcMax + 0.01, 0.1,
                    " Resolution : " + String.format("%.3f", 2 * pps / ida.kcMax) + " [" + units + "]");
