...
public class ImageDecorrelationAnalysis {
    ...
    ImageDecorrelationAnalysis(ImagePlus im, double rmin, double rmax, int Nr, int Ng, boolean doPlot,
            String savePath) {
        this.im = im;
        this.rmin = rmin;
        this.rmax = rmax;
    ...
    public void runAnalysis() {
        ...
        // IJ.log("Processing image : # slices " + nS + ", # frames " + nF + ", #
        // channels " + nC );
        ...
        for (int s = 1; s <= nS; s++)
            for (int f = 1; f <= nF; f++)
                for (int c = 1; c <= nC; c++) {
                    ...
                    ImagePlus imp = new ImagePlus(stack.getSliceLabel(this.im.getCurrentSlice()),
                            stack.getProcessor(this.im.getCurrentSlice()));
                    ...
                    // IJ.log("Input image preprocessing (W : " + imRef.getWidth() + ", H : " +
                    // imRef.getHeight() + ")");

                    // pre-processing steps : Apodization, make square image with a 2^N size
                    // Make FloatProcessor
                    this.imRef = this.imRef.toFloat(0, null);
                    FloatProcessor imf = new FloatProcessor(this.imRef.getWidth(), this.imRef.getHeight(),
                            (float[]) this.imRef.getPixels());
    ...
    private void computeD(ImageProcessor Ip) {
        ...
        for (int refine = 0; refine < 2; refine++) {
            for (int k = 0; k < this.Ng; k++) {
                ...
                double sig = Math.exp(
                        Math.log(gMin) + (Math.log(gMax) - Math.log(gMin)) * ((double) k / ((double) this.Ng - 1)));
            ...
            if (refine == 0) {
                ...
                if (ind2 < this.Ng) {
                    double gTemp = Math
                            .exp(Math.log(gMin) + (Math.log(gMax) - Math.log(gMin)) * (ind1 / ((double) this.Ng - 1)));
                    gMax = Math
                            .exp(Math.log(gMin) + (Math.log(gMax) - Math.log(gMin)) * (ind2 / ((double) this.Ng - 1)));
                    gMin = gTemp;
    ...
    private void computeD0(FloatProcessor[] Ir) {
        ...
        // long t1 = System.currentTimeMillis();
        FloatProcessor[] I = utils.normalizeFFT(Ir);
        // IJ.log("Normalize FFT time computation time : " +
        ...
        for (int k = 0; k < this.Nr; k++) {
            ...
            // IJ.log("Computation of a single cc time computation time : " +
            // (System.currentTimeMillis()-t1));
    ...
    // private double getCorrCoef(float[] pixelsIrR, float[] pixelsIrI, float[]
    // pixelsIR, float[] pixelsII, double r, double cr) {
    //
    // double d = 0;
    // double c = 0;
    // double r2 = r*r;
    ...
    private double[] getCorrCoefRing(float[] pixelsIrR, float[] pixelsIrI, float[] pixelsIR, float[] pixelsII,
        ...
        outerloop: for (int xx = ox; xx < ox + w; xx++)
